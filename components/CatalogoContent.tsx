...
import { automatizaciones, categoriasAutomatizacion, modalidadesAutomatizacion } from "@/lib/automatizaciones-data"
import type { ApiResponse, Automatizacion, Equipo, Sala } from "@/lib/types"
...
export function CatalogoContent() {
  ...
  const { data: automatizacionesResponse, isLoading: loadingAutomatizaciones } = useSWR<ApiResponse<Automatizacion[]>>(
    mostrarAutomatizaciones ? "/api/automatizaciones" : null,
    fetcher,
  )
  const salasDisponibles = salasResponse?.success && Array.isArray(salasResponse.data) ? salasResponse.data : []
  const automatizacionesDisponibles = automatizacionesResponse?.success && Array.isArray(automatizacionesResponse.data) ? automatizacionesResponse.data : automatizaciones
