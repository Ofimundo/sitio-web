...
// ─────────────────────────────────────────────
//  Página principal: Cotización de automatización
// ─────────────────────────────────────────────
...
export default function CotizarAutomatizacionPage() {
  ...
  if (submitted) {
    return (
      <SuccessMessage
        ...
        backHref="/catalogo?tipo=Automatización"
        backLabel="Volver al catálogo"
      />
    )
  }

  // ── Render: aside contextual (plan seleccionado)
  const selectedPlan = item.planes.find((plan) => plan.nombre === needs.plan)

  const contextualAside = selectedPlan ? (
    <div className="rounded-2xl border-2 border-ofimundo-purple bg-white p-6 shadow-sm">
      <p className="text-xs font-bold uppercase tracking-wider text-ofimundo-magenta">
        Plan seleccionado
      </p>
      <h2 className="mt-1 text-xl font-bold text-ofimundo-navy">
        {selectedPlan.nombre}
      </h2>
      <p className="mt-3 text-sm leading-relaxed text-gray-600">
        {selectedPlan.descripcion}
      </p>
      <h3 className="mt-5 font-semibold text-ofimundo-navy">Incluye</h3>
  ...
  return (
    <QuotationPage
      title={`Cotiza ${item.nombre}`}
      description="Cuéntanos cómo funciona hoy tu proceso para recomendarte el alcance y plan más adecuados."
      step={step}
      detailHref={`/automatizaciones/${slug}`}
      contextualAside={contextualAside}
    >
      <form onSubmit={submit} className="flex flex-col gap-6">
        {/* ── Paso 1: Necesidades ─────────────── */}
        {step === 1 ? (
          <div className="rounded-2xl border-2 border-ofimundo-purple bg-white p-6 shadow-sm md:p-8">
            {/* Encabezado de la automatización */}
            <div className="mb-6 flex items-start gap-4">
              <span className="flex size-12 shrink-0 items-center justify-center rounded-xl bg-ofimundo-purple text-white">
                <i className={`fas ${item.icono}`} />
              </span>
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-ofimundo-magenta">
                  Automatización seleccionada
                </p>
                <h2 className="mt-1 text-2xl font-bold text-ofimundo-navy">
                  {item.nombre}
                </h2>
              </div>
            </div>
            ...
            <div className="grid gap-6">
              ...
              <Field
                id="plan-help"
                label="¿Qué plan deseas cotizar? *"
                help="Si aún no estás seguro, selecciona Necesito asesoría y dimensionaremos la alternativa contigo."
              >
                <select
                  required
                  aria-describedby="plan-help"
                  value={needs.plan}
                  onChange={(e) => setNeed("plan", e.target.value)}
                  className={fieldClass}
                >
                  <option value="">Selecciona una opción</option>
                  {item.planes.map((plan) => (
                    <option key={plan.nombre}>{plan.nombre}</option>
                  ))}
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <Field
                  ...
                  help={`Puedes usar una referencia como ${item.metricas[0].toLocaleLowerCase()}.`}
                >
                  <input
                    required
                ...
                <Field
                  id="empresas-help"
                  label="Cantidad de empresas o RUT *"
                  help="Permite dimensionar reglas, accesos e integraciones."
                >
                  <input
                    required
              ...
              <Field
                id="sistemas-help"
                label="¿Qué sistemas debemos integrar? *"
                help={`Considera ERP, bancos, APIs u otros sistemas. Habituales: ${item.integraciones.slice(0, 3).join(", ")}.`}
              >
                <input
                  required
              ...
              {/* Personalización + Plazo */}
              <div className="grid gap-6 md:grid-cols-2">
                <Field
                  id="personalizacion-help"
                  label="Nivel de personalización"
                  help="Indica si necesitas reglas, reportes o notificaciones propias."
                >
                  <select
                    aria-describedby="personalizacion-help"
                    value={needs.personalizacion}
                    onChange={(e) =>
                      setNeed("personalizacion", e.target.value)
                    }
                    className={fieldClass}
                  >
                    <option>Por definir</option>
                    <option>Configuración estándar</option>
                    <option>Reglas personalizadas</option>
                    <option>Integración y flujo a medida</option>
                  </select>
                </Field>
                ...
                <Field
                  ...
                  help="Una referencia ayuda a organizar evaluación, integración y puesta en marcha."
                >
                  <select
                    required
                    aria-describedby="plazo-help"
                    value={needs.plazo}
                    onChange={(e) => setNeed("plazo", e.target.value)}
                    className={fieldClass}
                  >
              ...
              <Field
                ...
                label="Contexto u observaciones"
                help="Describe el proceso actual, sus dificultades y el resultado esperado."
              >
                <textarea
